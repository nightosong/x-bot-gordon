# 丹青溢彩项目工作流

## 项目资产

丹青溢彩的核心资产是 `ComicProject`：

- `summary`：故事与画面目标，回答这部漫画讲什么、看点是什么、读者应获得什么情绪。
- `visualStyle`：画风与镜头，回答线条、色彩、角色造型、构图、镜头运动和一致性规则。
- `episodePlan`：海报构图规划或连载总规划，回答项目如何分章、分镜和持续出图。
- `source`：来源元数据，记录原小说/文本来源、导入状态、章节数量和失败/部分导入说明。
- `assets`：素材库，包含人物、物品和场景。
- `chapters`：章节，每章有标题、章节内容简介、可选章节正文/故事内容、章节级提示、`storyboards` 分镜轨道、图片和素材引用。

Agent 不能把丹青溢彩当成单次图片生成器。它应把每次生成当成项目推进：补项目目标、补素材、拆章节、生成图片、审核一致性、写回状态。

## 用户交互模型

用户通常会这样使用：

1. 建一个漫画项目，写下类型、故事目标和画风。
2. 在素材库里维护主要人物、道具和场景。
3. 规划章节目录。
4. 选中一章，基于章节简介/正文拆成多条 `storyboards`。
5. 编辑每条分镜的画面、对白、镜头和生图提示词。
6. 对当前分镜生成单张图、漫画页、封面或连续图，并挂到该分镜。
7. 查看图片，修改分镜提示词或替换/追加当前分镜图片。
8. 根据成图继续完善下一章。

因此 Agent 输出必须贴近当前界面：

- 项目级结果写入 `summary / visualStyle / episodePlan`。
- 章节级结果写入 `title / summary / content / prompt / storyboards / status / assetRefs`，其中 `summary` 是章节内容简介，`content` 是可选正文或小说转漫画的故事文本，`prompt` 是章节级分镜提示，`storyboards` 是真正可出图的分镜轨道。
- 图片结果写入 `chapter.images`，并尽量通过 `storyboardId` 关联到当前分镜。
- 素材结果写入 `assets`，不要塞到章节备注里。

## 小说转漫画导入流程

当用户给出线上小说、上传文本、已有书稿或“把这本小说转成漫画”时，先按命令工坊事务处理，不直接进入出图：

1. 读取或研究来源，先确认目录页、章节链接、章节数量、来源标题和可读取状态。
2. 若来源可读，抽取章节目录；若正文量大，按批次导入，例如每 10 章一批。
3. 使用 `comic_create_project` 新建丹青溢彩项目，写入：
   - `source.sourceUrl / source.sourceTitle / source.chapterCount / source.extractionStatus`
   - `summary`：故事改编目标，不是原文大段粘贴。
   - `visualStyle`：全局视觉圣经，包括高层画风、角色统一风格提示词、场景统一风格提示词、镜头规则和禁忌。
   - `episodePlan`：导入批次、素材提取批次、分镜推进节奏和出图顺序。
4. 使用 `comic_import_chapters` 批量写入章节。原文或故事内容进入 `chapter.content`，章节内容简介进入 `chapter.summary`，来源引用进入 `chapter.sourceRefs`。
5. 完成章节导入后，再分批提取素材。人物、场景、物品写入 `assets`，角色服装或场景状态变化用：
   - `variantLabel`
   - `chapterStartIndex`
   - `chapterEndIndex`
   - `variants[]`
6. 准备好素材后，才按章节进入 `storyboard_director`，把一章拆成多条可编辑分镜，再按分镜出图。

### 逐章对照协议

如果用户要求“按章节访问”“把每章内容对照到章节里”“补全分镜提示词”，必须按下面顺序处理：

1. 先读取项目和来源目录，确认章节总数、已有导入范围和下一批需要访问的章节。
2. 逐章访问原文章节页。每一章都要记录：
   - 原文章节 URL。
   - 原文章节标题。
   - 原文章节序号。
   - 可读状态和备注。
3. 对每一章写入 `ComicChapter`：
   - `title`：优先使用原文章节标题；若原文无标题才使用“第 N 章”。
   - `sourceRefs`：至少包含一条 `sourceType=chapter` 的来源引用。
   - `content`：原文或忠于原文的章节故事内容。不要把工具执行说明、保留取舍、修改计划写进正文。
   - `summary`：中文章节内容简介，只概括本章发生的事件、角色目标、冲突变化和结尾钩子。
   - `prompt`：中文章节级分镜提示，写图片数量建议、画面节点、景别、构图、素材引用和一致性限制。
   - `storyboards[]`：如果本章已经准备进入分镜，把每张图/每格拆成可编辑分镜。
   - `assetRefs`：引用本章实际出现或需要保持一致的素材。
4. 从当前批次章节中抽取素材：
   - 人物、物品、场景都必须有原文依据，写入 `assets[].sourceRefs`。
   - 角色外观或场景状态随章节变化时，使用 `variants[]` 和章节范围，不要覆盖旧版本。
   - 多人/关系素材不要写成单人三视图。
5. 写回后必须读回验证：章节数量、目标章节的 `sourceRefs/content/summary/prompt/storyboards/assetRefs` 是否存在，素材是否进入项目素材库。

逐章对照失败时：

- 如果网页不可读、章节缺失或正文被反爬，不要生成虚假章节内容。
- 可以先写入目录占位和 `sourceRefs.note`，但 `content / summary / prompt / storyboards` 必须标明待补或保持为空。
- 最终回复要说明哪些章节已完成、哪些章节无法读取、下一步需要用户提供什么。

命令工坊和丹青溢彩的边界：

- 命令工坊负责来源抓取、目录统计、章节正文提取、项目创建、批量导入、素材候选提取和执行计划。
- 丹青溢彩负责项目编辑、视觉圣经维护、素材图片生成、分镜轨道编辑、分镜图片生成和视觉连续性审核。

如果网页遇到 Cloudflare、登录限制、反爬、版权限制或正文不可读，不要生成虚假章节数。应把状态标为 `blocked` 或 `partial`，并给用户明确下一步：换来源、使用浏览器读取、上传 txt/epub/docx、粘贴目录或粘贴章节正文。

## 写回策略

默认策略：

- 讨论、审阅、构思：不写回。
- 用户说“帮我生成并替换/保存/写入/落到项目”：可以写回。
- 用户说“生成几张图放到这一章”：可以调用 `image_gen` 并用 `comic_update_chapter_images` 写回。
- 用户说“把这一章拆成 20 张分镜图”：先用 `comic_update_chapter` 写回 20 条 `storyboards`，再按当前分镜逐条出图。
- 用户说“完善人物素材/场景素材”：优先用 `comic_update_assets` 写回。

写回前应尽量读取最新项目并带上 `expectedProjectUpdatedAt`。

## 项目推进质量

好的漫画项目结果必须同时满足：

- 读者能看懂故事进展。
- 模型能生成稳定画面。
- 用户能在界面里继续编辑。
- 后续章节能复用素材和视觉约束。

差的结果包括：

- 只有抽象美术词，没有角色和动作。
- 只描述剧情，没有画面和镜头。
- 只生成图片，没有保留提示词。
- 只给“修改说明/保留取舍”，没有生成可编辑的 `storyboards`。
- 素材库和章节分镜/出图提示互相矛盾。
- 连续图每张都像独立插画，没有动作推进。
