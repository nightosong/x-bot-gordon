...
| 桌面端视觉统一 | 已完成七版 | 已收敛为 Cyber Signal Deck（赛博信号甲板）方向：近黑网格基底、电青 / 冰蓝结构线、薄荷状态信号、翡翠薄荷主操作、琥珀告警、HUD 标题带和 8px 以内紧凑面板；当前已完成同色系配色收敛，整体底色维持 graphite / void black，列表、输入、导航和选中态统一使用电青 / 薄荷反馈，主操作使用更深饱和的翡翠薄荷，琥珀承接关键状态；已覆盖首页、应用广场、任务、流程、命令、模型与能力拓展，并对流程中心、任务笔记和应用广场内部工作区补齐编号轨、类型信号、侧轨质感和编辑区 HUD 表面 |
...
| 首页机器人工作区 | 已完成 | 已切换为本地 `@splinetool/runtime` 方案，机器人舞台在保留原深色背景的基础上增加白色飞星 canvas 背景层 |
| 模型管理 | 已完成 | 已支持配置列表、供应商切换、编辑、优先模型、余额查询、用量统计、Chat Completions / Responses 接口格式、流式输出开关与真实文本调用；配置列表已收敛为紧凑数据行，默认模型改为同组线性星芒状态按钮，避免与编辑、删除操作形成笨重的三块式控件；多模态生成不进入模型管理，改由能力拓展 TOOL 配置承接 |
| 模型配置持久化 | 已完成 | 当前走本地 JSON 仓储 |
| 任务笔记 | 已完成四阶段 | 已形成“本周驾驶舱 + 历史记录 + 项目推进/汇报双视图”的当前形态，列表态强化本周 / 历史记录与 KPI 信号层级，项目推进任务树加入项目级结构轨、任务状态节点和紧凑 HUD 行；状态菜单外部关闭、单菜单展开和状态节点视觉区分均已保留，支持待开始、进行中、测试中、受阻和已完成状态；汇报视图支持日报、周报、按日期范围生成述职报告，以及工作日北京时间 18:30 自动生成今日日报并发送到飞书群；新周报会继续带入上周未完成任务，包括待开始、进行中、测试中和受阻状态 |
...
- macOS 桌面端主窗口已隐藏原生长标题栏文字，并用近黑顶部拖拽带衔接 Cyber Signal Deck，避免系统白色标题栏割裂主界面
...
- 首页机器人区域已使用本地 Spline runtime，不依赖在线 viewer；机器人背后新增白色飞星 canvas 背景层，参考 Falling Stars 的透视星点与拖尾效果，但继续沿用 Gordon 原深色背景
- 桌面端视觉已收敛为 Cyber Signal Deck（赛博信号甲板）方向，统一近黑网格基底、冰蓝结构线、薄荷状态信号、翡翠薄荷主操作、琥珀警示与关键数据，并以扫描纹理和 HUD 刻度建立机械界面氛围
- Cyber Signal Deck 七版完成同色系配色收敛：移除粉红色和离子蓝高亮，底色保持更纯的 graphite / void black，结构线维持冷电青 / 冰蓝，主按钮改用更深饱和的翡翠薄荷，当前轨道、列表选中和焦点统一使用电青 / 薄荷反馈，默认模型、金融与关键数据保留琥珀暖光
- 当前视觉统一已覆盖左侧品牌与主导航、首页机器人 HUD、页面标题带、应用入口、KPI、列表项、配置项、按钮、表单控件、详情工具栏和自绘窗口，不改变左侧入口 / 右侧主工作区架构
- 内部工作区已补齐 Cyber Signal Deck 的第二层视觉语言：流程中心入口卡使用编号与类型色轨区分信息雷达、金融、直播和接口测试；任务笔记强化周记录信号轨、项目结构轨和状态节点；应用广场入口、项目架、详情侧轨、Tab 和编辑面板统一使用应用编号、资产边界、暖金字段标签和近黑 HUD 表面
...
- 命令工坊高级设置中的 Agent、Skill、工具服务和工具选择已从原生下拉框切换为 Gordon 自绘紧凑下拉控件，选项弹层统一使用 Cyber Signal Deck 的近黑 HUD 表面、冰蓝结构边界、薄荷焦点和紧凑列表风格
...
- `信息雷达` 交互与内容覆盖已升级：搜索来源改为聚合 Google News RSS + Bing RSS 双源抓取并去重合并，扩大最新技术 / 前沿资讯的覆盖面；信息窗口新增"前沿科技 / AI 前沿 / 开发者 / 科研前沿"四套预设，空态与编辑器均提供一键套用（预设仅作为初始草稿注入，保存后仍归属用户配置，不做项目级硬编码窗口）；信息主舞台加入雷达脉冲实时状态、标题脉冲指示与更酷炫的深色玻璃预设启动器，信息条目卡片整卡可点直接进入软件内阅读页，键盘可聚焦打开；相关图标经 `GIcon.vue` 语义层扩展（radar / rocket / code / flask / globe / zap / bookmark）
...
- 后续视觉迭代继续沿用 Cyber Signal Deck，不切换为浅色基底、满屏大圆角玻璃卡或单一紫蓝霓虹风格；赛博装饰必须服务层级、状态或交互反馈
