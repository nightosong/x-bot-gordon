...
| 任务笔记 | 已完成三阶段 | 已形成“本周驾驶舱 + 历史记录 + 项目推进/汇报双视图”的当前形态，项目推进任务树已优化状态菜单外部关闭、单菜单展开和状态节点视觉区分，汇报视图支持日报、周报、按日期范围生成述职报告，以及工作日北京时间 18:30 自动生成今日日报并发送到飞书群；新周报会继续带入上周未完成任务，包括待开始、进行中和受阻状态 |
| 命令工坊 | 已完成首版 | 已支持多轮会话、默认 Agent、附件上传、按需工具调用和轻量过程流展示；简单问答不再展示开场思考 / 执行计划卡片，真实工具执行、授权、参数修复、重试 / fallback、停止原因和少量必要输出才进入可见时间线；成功工具调用默认只展示动作标题、目标摘要和状态标签，原始返回保留在 artifact 中，错误、授权异常和停止原因才提供可展开输出，并按 harness Agent 语义区分本地工具和外部 MCP；输入区已新增访问权限下拉，默认 `无需申请`，当前 Agent 运行会自动授予外部路径、Computer Use 和高风险工具的本轮运行时权限，切换为 `按需申请` 后才在执行前弹出 Gordon 授权确认；运行中队列项点击“引导”会进入当前运行上下文，若引导在最终回复流式生成期间到达，Runtime 会撤回当前草稿、重新评估工具需求并按需补工具结果后再生成最终回复；默认 Agent 已接入 Workspace Tools、Search Tools、Computer Use、Gordon Tools 与 Application Tools，可进行高质量联网研究、GitHub 仓库搜索、桌面应用读取 / 控制、发现已启用的 TOOL 配置，并可新建或联动应用广场的墨笔生花小说资产；自动工具循环已引入任务账本、Resource Registry、Resource Gateway、Context Packet、Capability Routing、Task Continuation Engine、Tool Requirement Engine、Decision Memory、Plan Critic、Evidence Graph、决策轨迹、分层观察和结构化成功条件，Runtime 会先判断本轮是新任务、延续任务还是方向变化，再判断是否必须用工具；Verifier 优先用工具历史确定性验证成功条件，只有剩余明确、可行动的硬断言才进入主动验证规划，弱条件或已有产物证据不会再触发 12 秒收尾规划；required 场景下 Planner 不能直接 `shouldCall:false`，Planner 拒绝、无效或超时时会进入 required fallback 或 safe failure；同一会话会把上一轮助手消息 artifact 中的 `taskLedger` 注入下一轮 Agent 请求，明显的“继续 / 接着做”可由账本直接驱动下一步执行而不重新规划整个任务；当前以 Gordon 持续执行型工程 Agent 为唯一默认内置 Agent，科研协作通过 `arthur-research` Skill 加载，显然需要工具的任务会自动进入工具编排和验证闭环；交互流畅度已对齐 Codex / Claude Code 的第一批体感：流式输出仅在用户贴底时自动跟随、上滚回看不再被强制拉回底部并提供「回到底部」悬浮按钮，最终回复流式阶段改为纯文本 + 闪烁光标承接增量（结束后再走完整富文本渲染，避免每帧整段 Markdown re-parse 抖动），运行中 Enter 默认即时插话当前运行、仅在无法注入时退回请求队列，输入框支持 Esc 停止本轮运行、↑ 在空输入时召回上一条已发送请求；第二批改造已让规划 / fallback / 观察压缩 / 主动验证等中间模型调用透出带计时的轻量「思考态」心跳（thinkingText），长任务静默等待持续可见，运行失败 / 停止改为内联提示 + 「重试本轮」（保留入参）与「从这里继续」（复用 taskLedger 续跑）入口，命令工坊内 Cmd / Ctrl + K 可快速新建会话；第三批改造已加入输入框 / 斜杠命令面板（过滤 + 上下选 + Enter 执行 + Esc 关闭），内置 new / settings / skill / agent / tools / 权限 / attach / stop / copy / list 等命令，等待状态文案加 shimmer 流光 |
...
- 流程中心渲染层已拆出 `features/workflow-library/WorkflowLibraryView.vue`、`features/workflow-library/workflowRuntime.js` 与 `features/workflow-library/workflowActions.js`：页面组件承接流程中心首页、信息雷达主界面、信息窗口编辑器、工作流列表、编辑器和执行态模板；runtime 集中承接信息雷达展示 helper、curl 解析、请求 Body 修复、环境归一化、记录草稿转换、运行前缺失项检查和运行结果展示 helper；actions 承接选择同步、信息窗口保存 / 删除 / 刷新、记录保存 / 复制 / 删除、环境与 Body 写回、执行 / 中断和 IPC 进度回显；`App.vue` 当前只保留顶层状态注入和事件注册
