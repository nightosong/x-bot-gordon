...
- 命令工坊输入区附件按钮右侧已新增 `访问权限` 紧凑下拉，默认 `无需申请`；本轮 `AgentRunRequest.permissionMode=auto` 会让 Runtime 自动通过外部路径、Computer Use 与高风险工具授权，不弹出确认窗口，也不会把授权写入长期配置；切换为 `按需申请` 后恢复逐次授权确认。
...
- 命令工坊已加强口语查询与笔误容错：`查下 / 查查 / 查一查` 会触发工具编排，Anthropic / Claude 等厂商当前模型清单、官方价格和最新资料会被 Runtime 强制归入外部证据任务；对“旗下罪行的模型”这类明显接近“旗下最新的模型”的输入，会在搜索 query 中归一为最新模型意图，并优先使用官方英文 canonical query、官方域名过滤与官方 pinned source URL；如果搜索命中无关页面、官网首页或缺少价格 / 模型实体证据，Runtime 会自动补搜而不是让最终回复用旧知识兜底。
