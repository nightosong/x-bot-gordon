...
| 流程中心 | 已完成四阶段 | 已从“效率工具”扩展为信息工作流中心：新增 `信息雷达` 卡片，支持维护多个信息窗口、RSS / 网页来源、关键词 / 排除词、摘要规则、刷新历史和信息流展示；模型接口测试继续支持动态请求步骤、环境切换、环境级 APIKEY 注入、请求 Body 快捷编辑、响应变量提取、步骤级轮询、实时执行输出、可中断执行、可视化执行状态与折叠式请求步骤；流程中心页面动作已迁入 `workflowActions.js` |
...
- 首页采用黄金比例横向 workflow 卡片，每行 3 张；进入不同卡片后切到对应资源工作区，避免把信息流、curl 列表和配置项混在同屏
- 当前默认卡片为 `信息雷达` 与 `模型接口测试`
- `信息雷达` 当前支持多个信息窗口，窗口内可维护 RSS、网页、搜索、公众号和手工来源，其中 RSS / Atom 与普通网页可由桌面主进程手动刷新；刷新会按关键词和排除词过滤、去重、写入信息条目、保留 run history，并在页面内以左侧窗口轨道 + 右侧信息流主舞台展示。搜索与公众号来源当前先保存配置和边界提示，后续接 Search Tools 与公众号专项采集
