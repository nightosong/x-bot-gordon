...
| 首页与导航 | 已完成 | 采用“左侧 mini 品牌卡 + 紧凑功能入口 + 右侧单一主工作区”结构，左侧导航宽度收窄，主工作区保留更大显示面积 |
...
| 任务笔记 | 已完成三阶段 | 已形成“本周驾驶舱 + 历史记录 + 项目推进/汇报双视图”的当前形态，项目推进任务树已优化状态菜单外部关闭、单菜单展开和状态节点视觉区分，支持待开始、进行中、测试中、受阻和已完成状态；汇报视图支持日报、周报、按日期范围生成述职报告，以及工作日北京时间 18:30 自动生成今日日报并发送到飞书群；新周报会继续带入上周未完成任务，包括待开始、进行中、测试中和受阻状态 |
...
- macOS 桌面端主窗口已隐藏原生长标题栏文字，并用深色顶部拖拽带衔接 Gordon Dark Glass Workbench，避免系统白色标题栏割裂主界面
