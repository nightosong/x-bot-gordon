...
| 命令工坊 | 已完成首版 | 已支持多轮会话、默认 Agent、附件上传、按需工具调用和按需过程流展示；简单问答不再展示开场思考 / 执行计划卡片，真实工具执行、授权、参数修复、重试 / fallback、中间输出和停止原因才进入可见时间线，并按 harness Agent 语义区分本地工具和外部 MCP；默认 Agent 已接入 Workspace Tools、Search Tools、Computer Use、Gordon Tools 与 Application Tools，可进行高质量联网研究、GitHub 仓库搜索、桌面应用读取 / 控制、发现已启用的 TOOL 配置，并可新建或联动应用广场的墨笔生花小说资产；自动工具循环已引入任务账本、Resource Registry、Resource Gateway、Context Packet、Capability Routing、Decision Memory、Plan Critic、Evidence Graph、决策轨迹、分层观察和结构化成功条件，工具规划每轮基于 `goal / resources / activePlan / decisionMemory / evidence / verification / recovery / openQuestions` 与能力路由提示决定下一步，并按 Resource -> Capability -> Tool 的顺序记录目标资源、能力步骤、预期结果与验证方式；同一会话会把上一轮助手消息 artifact 中的 `taskLedger` 注入下一轮 Agent 请求，支持长期任务跨轮延续；当前以 Gordon 持续执行型工程 Agent 为唯一默认内置 Agent，科研协作通过 `arthur-research` Skill 加载，显然需要工具的任务会自动进入工具编排和验证闭环 |
...
- 命令工坊运行态展示已对齐持续执行型 Agent 体验：Planner Tool View、工具数量和工具发现摘要只作为内部 artifact 保留，主消息顶部统一展示“准备工具 / 判断动作 / 执行工具 / 整理回复”的稳定 live 状态；真实工具步骤会转成面向任务的动作标题（例如 `read_web_page` 显示为“读取网页内容”），工具服务和 tool name 降级为小标签，避免用户看到卡片从“工具数量”突然跳到“Workspace Tools / read_web_page”
...
- 支持会话内选择 Agent、附加 Skill、自动工具、限定工具服务 / tool；运行中可继续输入并发送到请求队列，队列在输入框上方按顺序展示，当前轮完成后自动进入下一轮；队列项支持点击“引导”直接进入对话并静默接入下一轮 Agent 思考，也支持编辑和删除
