...
- `信息雷达` 当前支持多个用户自定义信息窗口，窗口内可维护 RSS、网页、搜索、公众号和手工来源，其中 RSS / Atom、普通网页、公开搜索 RSS 与搜狗微信公众号线索可由桌面主进程手动刷新；刷新会按关键词和排除词过滤、去重、写入信息条目、保留 run history，并在页面内以可折叠左侧窗口轨道 + 紧凑来源 / 标签筛选、相关性分数和普通信息流主舞台展示；点击条目来源会进入软件内来源阅读页，上层只显示返回、单行标题和外部打开，下方由主进程 `BrowserView` 内嵌网页内容，返回后销毁阅读视图并回到原信息列表。信息雷达初始化不预置具体信息窗口，Agent / 大模型、科研论文、金融、生物学等窗口属于用户配置，统一落在 `~/.gord/data/workbench/workflow-library.json`；公众号来源依赖公开搜索页，遇到限流会在刷新记录中提示
...
- 为模型配置补齐连接测试或校验能力
- 为流程中心继续补齐信息雷达能力：定时刷新、Search Tools 深研接入、公众号稳定采集 / 订阅源、信息条目保存到 Memory / 任务账本、以及由命令工坊触发的“资料收集 -> 信息窗口沉淀 -> 后续研究”链路
