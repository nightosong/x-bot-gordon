...
- `丹青溢彩` 漫画项目动作已拆入 `features/marketplace/comicActions.js`，包括项目归一化、项目架动作、章节编辑、分镜轨道编辑、自动保存、删除与 Markdown 导出；章节字段语义已收敛为“章节内容简介 summary / 可选章节正文 content / 章节级分镜提示 prompt / 分镜轨道 storyboards / 图片 images”，OUTLINE 页右侧按“章节内容简介 / 章节正文 / 分镜与出图提示”三段统一折叠编辑，标题默认保存为“第 N 章”并采用就地编辑，编辑为空时恢复编辑前标题；CHAPTER 页以分镜轨道为主体，支持新增 / 选择 / 删除分镜，编辑每条分镜的标题、类型、画面内容、对白/旁白、镜头/构图和生图提示词，灵绘小筑可按目标分镜数拆分章节并把图片写回当前分镜
...
- 命令工坊已引入首版 Capability Routing（能力路由）上下文：runtime 会基于 Context Packet 推断本轮能力需求，并按能力、动作、执行域、成本、风险、副作用和可逆性对工具做分组排序提示，注入 Planner、fallback planner 与主动验证规划；该上下文只做多候选工具检索和成本 / 风险提示，`allToolsAvailable` 保持完整候选工具可选，不做硬编码候选裁剪或强制路由
...
- Agent runtime 关键纯逻辑已从 `runtime.ts` 拆到 `ledger.ts`、`context-packet.ts`、`capability-router.ts`、`evidence-graph.ts`、`plan-critic.ts`、`tool-metadata.ts`、`failure-classifier.ts`、`verifier.ts` 与 `runtime-utils.ts`，并新增 `pnpm run test:agent` 覆盖工具描述清洗、工具画像、Capability Routing、失败分类、账本归一化 / 合并、Context Packet、Evidence Graph、Plan Critic、结构化成功条件验证、evidence 生成和首版 `runAgent` 端到端集成回归；当前集成回归会用本地假模型与假 MCP 服务验证自动工具规划链路保留完整候选集，并能选择 Computer Use 执行桌面状态读取
...
- 聊天态已支持执行中链路与最终回复流式回显，不再只在最终结果返回后一次性展示；运行刚开始且尚未出现过程项或正文时，会展示带脉冲与三点节奏的轻量等待态，避免页面只停在静态“正在处理”；运行中可主动停止，已流出的部分回复会保留在当前会话中；OpenAI-compatible / Azure 网关若在流式通道未产出正文前报错、返回空流，或以 JSON 错误体拒绝 `stream=true`，会自动降级重试普通非流式请求，并在当前桌面进程内把该模型标记为非流式优先，避免同一代理反复打断本轮会话；流式解析已区分真正增量与最终快照，防止部分网关在流末返回完整消息时把回复追加成重复文本
