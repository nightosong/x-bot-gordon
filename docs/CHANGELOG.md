...
- 全局图片放大预览已统一为根层蒙版交互，支持下载、关闭、缩放比例显示、按钮 / 滚轮缩放和底层操作阻断；漫画素材、漫画 AI 输出图与命令工坊图片产物优先接入同一套预览层
...
- TOOL 配置当前内置 `image_gen`、`video_gen`、`music_gen`，分别维护图片、视频、音乐生成能力的供应商启停、默认供应商、API Key、Base URL、模型 / 能力 ID 与备注；供应商范围先覆盖 OpenAI / Gemini / 即梦、Seedance / PixVerse / Veo / Sora、Mureka / Suno；`image_gen/openai` 已内置文生图 `imagen`、图生图 `imagen/edit` 端点与 `prompt / model / size / n / quality / image / images` 参数边界，运行时会按单图 `image`、多图 `image[0] / image[1]...` 组装 multipart 图生图请求；`video_gen/seedance` 已内置火山方舟官方 `POST /api/v3/contents/generations/tasks` 与 `GET /api/v3/contents/generations/tasks/{task_id}`，支持文生视频、图生视频、首尾帧生视频、参考图 / 视频 / 音频生视频、异步查询、默认短轮询和无完成结果时的 `pending` 状态回传，Base URL 归一化兼容旧代理路径、完整官方任务路径和 `/api/v3` 前缀；`music_gen/mureka` 已内置 `/v1/song/generate`、`/v1/soundtrack/generate`、`/v1/song/query/{task_id}` 与 `/v1/vocal/clone`，`music_gen/suno` 已改为代理提交 `/gpt-proxy/suno/generate` 与 POST 查询 `/gpt-proxy/suno/detail`，生成和查询默认短轮询到音频 URL 或明确 pending / pollFailed；默认 Agent 可通过 `Gordon Tools` 发现并调用已启用的生成能力
