...
| 首页与导航 | 已完成 | 采用“左侧 mini 品牌卡 + 紧凑图标工具轨 + 右侧单一主工作区”结构；品牌卡移除独立 `G` 图标，恢复低频 `GORDON` 文字变化并在短暂停留后自动回归，主导航统一使用 Lucide 线性图标、双语短标签和激活轨道 |
| 桌面端视觉统一 | 已完成定稿 | 已从 Cyber Signal Deck 迁移为 Saber Atelier（二次元工作室）方向：首页使用 Saber 专用首页背景图，其它页面接入 Saber 聊天背景低频轮换；最终设计层由 `saber-atelier.css` 承接，统一为纸白主工作面、深墨文字、蓝色交互、暖金强调与少量淡紫识别点。非首页工作区移除旧玻璃渐变、装饰网格和多层外框，固定为“一个主纸面 + 分隔式内部层级”；KPI、模型与数据区域采用贴纸式指标和账本行，列表与按钮使用紧凑 4-6px 圆角、实色按钮和清晰焦点态。代码块、媒体舞台、封面图和播放器保留深色内容例外，原有工作台结构与业务交互不变 |
...
| 首页二次元舞台 | 已完成 | 已移除本地 Spline 机器人和飞星 canvas，首页改为 Saber 专用背景图 + 轻量品牌文字层；全局壳层通过 `SaberThemeBackdrop.vue` 在非首页使用 Saber 聊天背景轮换，并在左侧功能 tab 切换时通过缓冲防抖、最短停留和旧图稳定淡出 / 新图柔和淡入的叠层过渡只推进一次背景；聊天背景已改用 1080p WebP 运行图、启动阶段少量预热、首页或窗口隐藏时暂停轮换，降低 Electron 启动与切页阶段的图片解码压力 |
| 模型管理 | 已完成 | 已支持配置列表、供应商切换、编辑、优先模型、余额查询、用量统计、Chat Completions / Responses 接口格式、流式输出开关与真实文本调用；模型列表采用连续名册纸面，默认模型以蓝金书签标识，条目收紧为 64px mini 行，仅保留身份、模型名、余额摘要和图标操作。未配置余额时不再渲染占位框；已配置余额收为单行账本摘要。数据统计采用连续记录纸面、分栏关键指标、蓝金用量图和账本行，避免独立卡片堆叠导致的低对比与工业感；多模态生成不进入模型管理，改由能力拓展 TOOL 配置承接 |
| 模型配置持久化 | 已完成 | 当前走本地 JSON 仓储 |
| 任务笔记 | 已完成四阶段 | 已形成“本周驾驶舱 + 历史记录 + 项目推进/汇报双视图”的当前形态，列表态强化本周 / 历史记录与 KPI 信号层级，项目推进任务树已收敛为浅色项目纸面、左侧状态细轨和语义状态徽标；状态色固定为待开始灰蓝、进行中青绿、测试中蓝色、受阻红珊瑚、已完成低饱和灰，金色只保留给时间、提醒和重要信号；状态菜单与更多操作菜单会在打开时提升当前任务与项目卡层级，并使用浅底高对比样式避免被相邻卡片或背景图干扰；状态菜单外部关闭、单菜单展开和状态节点视觉区分均已保留，支持待开始、进行中、测试中、受阻和已完成状态；汇报视图支持日报、周报、按日期范围生成述职报告，以及工作日北京时间 18:30 自动生成今日日报并发送到飞书群；新周报会继续带入上周未完成任务，包括待开始、进行中、测试中和受阻状态 |
| 命令工坊 | 已完成首版 | 已支持多轮会话、默认 Agent、附件上传、按需工具调用和轻量过程流展示；简单问答不再展示开场思考 / 执行计划卡片，真实工具执行、授权、参数修复、重试 / fallback、停止原因和少量必要输出才进入可见时间线；成功工具调用默认只展示动作标题、目标摘要和状态标签，原始返回保留在 artifact 中，错误、授权异常和停止原因才提供可展开输出，并按 harness Agent 语义区分本地工具和外部 MCP；输入区已新增访问权限下拉，默认 `无需申请`，当前 Agent 运行会自动授予外部路径、Computer Use 和高风险工具的本轮运行时权限，切换为 `按需申请` 后才在执行前弹出 Gordon 授权确认；运行中队列项点击“引导”会进入当前运行上下文，若引导在最终回复流式生成期间到达，Runtime 会撤回当前草稿、重新评估工具需求并按需补工具结果后再生成最终回复；默认 Agent 已接入 Workspace Tools、Search Tools、Computer Use、Gordon Tools 与 Application Tools，可进行高质量联网研究、GitHub 仓库搜索、桌面应用读取 / 控制、发现已启用的 TOOL 配置，并可新建或联动应用广场的墨笔生花小说资产；自动工具循环已引入任务账本、Resource Registry、Resource Gateway、Context Packet、Capability Routing、Task Continuation Engine、Tool Requirement Engine、Decision Memory、Plan Critic、Evidence Graph、决策轨迹、分层观察和结构化成功条件，Runtime 会先判断本轮是新任务、延续任务还是方向变化，再判断是否必须用工具；Verifier 优先用工具历史确定性验证成功条件，只有剩余明确、可行动的硬断言才进入主动验证规划，弱条件或已有产物证据不会再触发 12 秒收尾规划；required 场景下 Planner 不能直接 `shouldCall:false`，Planner 拒绝、无效或超时时会进入 required fallback 或 safe failure；同一会话会把上一轮助手消息 artifact 中的 `taskLedger` 注入下一轮 Agent 请求，明显的“继续 / 接着做”可由账本直接驱动下一步执行而不重新规划整个任务；当前以 Gordon 持续执行型工程 Agent 为唯一默认内置 Agent，科研协作通过 `arthur-research` Skill 加载，显然需要工具的任务会自动进入工具编排和验证闭环 |；已完成沟通体验增强：运行结束或停止后焦点自动回到输入框（对齐 Codex 手感）；输入历史栈支持 ↑/↓ 浏览最多 50 条已发送请求；图片粘贴支持在输入框直接 Cmd+V 粘贴截图作为附件；AI 自动标题在首轮回复后异步生成会话标题（5-12 字）；会话支持双击重命名、置顶（pin）排序和关键词搜索过滤；消息支持内联编辑并截断历史重新提交（edit & resubmit）；补全全局快捷键：Cmd+[ / Cmd+] 切换会话、Cmd+Shift+C 复制最后一条回复；过程流步骤支持点击轨道折叠/展开；工具结果支持 diff / 代码 / shell / 搜索类型专属结构化渲染
...
| 应用广场 | 已完成首版 | 当前已接入写作助手“墨笔生花”，支持应用入口、书架、书籍详情三 tab、章节化目录 / 编写状态、本地书稿持久化、结构化 storyAssets 故事资产、Narrative State 叙事状态图、长文本创作工作流式 AI 辅助、prompt 资产外置、黄金一章/三章自评、章节生成后的连续性资料写回和已完成章节书稿导出；项目型应用封面管理已从墨笔生花抽成通用能力，墨笔生花书籍、丹青溢彩漫画项目、流光绘影视频项目和瑶琴映月音乐专辑均可在详情侧栏上传 URL、本地图片或通过 `image_gen` 生成竖版封面，封面弹窗采用草稿预览 + 确认写回，本地上传不把 base64 写入 URL 输入框，预览卡片可悬浮下载，生成提示词复用字段级 AI 优化器并写回各自项目数据；墨笔生花已补充 `Genre Profile` 题材画像、证据化 storyAssets / Narrative State 和 `characterArcs` 人物弧线，并新增首版 Gordon 写作导演层：快速模式、长篇 Master Plan、分批目录规划和章节写回后的连续性更新都会先生成 `intent / phase / skillNodes / risks / successCriteria / conflictSeed / stateCommitPolicy`，再把 Gordon Agent Runtime 摘要、内置 `writing` Skill 技巧、轻量 Event Graph 和当前书稿状态注入模型；添香小筑已提供“Gordon 处理”入口，可通过 `buildWritingAgentRunInput` 构造默认 Gordon Agent + 内置 `writing` Skill + 自动工具编排的标准任务输入，结果先回填到输出区，不直接覆盖书稿；应用广场通用 Gordon 处理层已由 `marketplaceAgentActions.js`、`marketplaceAgentContext.js`、`AiAssistantActionBar.vue` 和 `GordonAgentProgress.vue` 承接，添香、灵绘、瑶音、流光和灵犀统一使用“状态 / 快速模式 / Gordon 处理”同排操作条，并复用同一套上下文构造、过程流、停止入口、artifact 解析和结果回填：快速模式走应用轻量模型或专用工具，Gordon 处理由默认 Gordon Agent 主导目标理解、质量判断、状态连续性、风险控制和必要工具调用；左侧信息轨保留类型维护入口，子类型和故事引擎继续作为底层题材画像数据参与提示词和工具写入，提示词会按探险、都市、言情、悬疑、历史、科幻、奇幻、武侠、现实等题材切换创作策略；写作提示词与写入链路已加入“作品成品纯净协议”，区分内部资产盘点 / 审稿报告与可写入作品字段的成品稿，简介、大纲、补充设定、章节简介和目录落盘前会拦截“上一版 / 本次调整 / 保留 / 降级 / 用户要求”等讨论痕迹，避免把创作过程写进小说设计；章节写回和命令工坊 Application Tools 写入长期事实时会保留 `evidenceRefs` 与 `impact`，人物变化按 want / need / currentStage / nextPressure / endpoint 维护，降低长篇后期人物漂移和状态失真；漫画创作应用“丹青溢彩”已支持应用入口、项目架、新建项目、项目详情三 tab、本地项目持久化、项目级素材库和灵绘小筑抽屉，总介绍 / 目录通过优先文本模型生成漫画介绍、画风规划、总规划或章节目录，素材库可维护人物（16:9 单张三视图）、物品（16:9 单张三视图）、场景（多图 / 多视角）并在单章生成中引用，素材库左侧列表可折叠，章节目录页已将新增章节收进标题栏图标按钮并隐藏左侧横向滚动；CHAPTER 页已把分镜轨收敛为窄序号列，分镜标题、类型、画面简介和状态信息回到中间 Canvas 与右侧 Shot Editor；单章生成通过 `image_gen` 生成单张漫画图、漫画页分镜、封面海报或多张连续图，并以左侧图片舞台 + 右侧选中图片参数 / 生图提示词联动面板展示章节内容；章节图片已从生成稿正文拆到 `ComicChapter.images`，每张图记录自身提示词、尺寸和质量，旧 Markdown 图片可自动迁移，data URL 图片会文件化沉淀到 `data/workbench/comic-images/`；视频生成应用“流光绘影”已支持应用入口、项目架、新建项目、项目详情三 tab、镜头规划、快速镜头方案、本地项目持久化和 `video_gen` 生成 / 查询一体化反馈，工具默认在内部短轮询到视频产物、pending 或 pollFailed；项目型详情页已统一把书名 / 项目名移到左侧折叠信息轨，顶部原标题区域用于承载 tab 导航，编辑区不再叠加二级标题卡，左侧单行输入和下拉控件已 mini 化并使用暖金色字段 label，标题 / 命名类输入支持临时清空并在 10 秒无新输入后回退；音乐创作应用“瑶琴映月”已升级为“应用入口 -> 专辑列表 -> 专辑编辑详情”，支持空列表提示、新建专辑、左侧可折叠专辑信息轨、顶部全部 / 草稿 / 成品 tab、播放器、曲目编辑、专辑 Markdown 导出、顶部瑶音小筑助手入口、快速制作草案，并可通过 `music_gen` 调用 Mureka / Suno 一次性提交并短轮询生成歌曲或纯音乐，若预算耗尽则保留 taskId 与 pending 状态供后续继续查询；占卜运势应用“灵犀照命”已升级为多流派对话问询工作台，左侧保留今日运势、综合看命、面相手相、阳宅风水、事业财运、感情关系、抉择占卜七个常用入口，右侧以聊天流承接用户问题、图片 / 资料附件、必要信息追问、象意取卦 / 盘名和可复盘解读，提示词按资料完整度、取象来源、卦名合理性、交叉印证、行动建议和复盘指标生成参考；主要长文本编辑框已接入字段级 AI 优化，可在字段旁调用优先模型并替换或追加回当前字段；应用广场主模板、写作基础动作、写作 AI 编排、漫画项目动作、漫画 AI 抽屉、视频项目动作、音乐创作动作、运势解读动作、应用 Gordon 处理层与字段优化动作已迁入 feature 模块 |
| 流程中心 | 已完成四阶段 | 已从"效率工具"扩展为信息工作流中心：`信息雷达` 支持用户维护多个信息窗口、RSS / 网页 / 搜索 / 公众号线索来源、关键词 / 排除词、摘要规则、刷新历史、左侧窗口轨折叠、来源 / 标签下拉筛选、普通信息流和相关性可视化，并支持在右侧主舞台内打开来源阅读页；阅读页由主进程 `BrowserView` 内嵌网页，顶部只保留返回、单行标题和外部打开，正文铺满右侧窗口并提供加载 / 失败兜底；公众号来源只做低频线索发现，冷却期跳过搜索页访问，条目长期只保存标题、作者、发布时间和摘要，点击打开时重新按标题 / 作者检索可用链接并在软件内阅读页加载；信息窗口和来源不再由项目代码硬编码预置，用户自己的 Agent、论文、金融、生物学等关注窗口统一落在 `~/.gord/data/workbench/workflow-library.json`；模型接口测试继续支持动态请求步骤、环境切换、环境级 APIKEY 注入、请求 Body 快捷编辑、响应变量提取、步骤级轮询、实时执行输出、可中断执行、可视化执行状态与折叠式请求步骤；流程中心页面动作已迁入 `workflowActions.js`；`信息雷达` 已聚合 Google News + Bing RSS 双搜索源扩大前沿资讯覆盖，新增四套前沿窗口预设一键套用，并加入雷达脉冲状态、整卡可点阅读和浅色玻璃预设启动器 |
| 流程中心 | 已完成五阶段 | 已从"效率工具"扩展为信息工作流中心：`信息雷达` 支持用户维护多个信息窗口、RSS / 网页 / 搜索 / 公众号线索来源、关键词 / 排除词、摘要规则、刷新历史、左侧窗口轨折叠、来源 / 标签下拉筛选、普通信息流和相关性可视化，并支持在右侧主舞台内打开来源阅读页；阅读页由主进程 `BrowserView` 内嵌网页，顶部只保留返回、单行标题和外部打开，正文铺满右侧窗口并提供加载 / 失败兜底；公众号来源只做低频线索发现，冷却期跳过搜索页访问，条目长期只保存标题、作者、发布时间和摘要，点击打开时重新按标题 / 作者检索可用链接并在软件内阅读页加载；信息窗口和来源不再由项目代码硬编码预置，用户自己的 Agent、论文、金融、生物学等关注窗口统一落在 `~/.gord/data/workbench/workflow-library.json`；模型接口测试继续支持动态请求步骤、环境切换、环境级 APIKEY 注入、请求 Body 快捷编辑、响应变量提取、步骤级轮询、实时执行输出、可中断执行、可视化执行状态与折叠式请求步骤；流程中心页面动作已迁入 `workflowActions.js`；`信息雷达` 已聚合 Google News + Bing RSS + arXiv Atom（学术类查询自动加入）三源抓取并去重合并，预设库已从 4 个扩充至 11 个，新增算法前沿、全球金融市场、A 股市场、政治外交、生物科技、农业食农、量子与物理七大领域预设，每个预设配有高质量 arXiv、Nature、Reuters、Bloomberg、FAO、USDA 等来源 RSS；每来源最大条数从 16 条提升至 32 条，每窗口最大存储从 160 条提升至 400 条，使多领域并行监控成为可能；并加入雷达脉冲状态、整卡可点阅读和浅色玻璃预设启动器 |
| 流程中心 | 已完成七阶段 | 已新增 `直播流` 默认卡片，流程中心形成 `信息雷达 / 金融快报 / 直播流 / 模型接口测试` 四板块；直播流默认内置用户关注的 Bilibili `https://live.bilibili.com/blanc/6` 纯净播放页，支持收藏 Bilibili 固定房间、小红书比赛直播页和自定义直播页，Bilibili 可输入房间号或完整 URL 并统一归一化为 `/blanc/<roomId>`，右侧通过独立 `BrowserView` 原生网页舞台播放，左侧保留直播间轨道和平台输入区；Bilibili 直播页会在加载后进入播放器剧场模式，尽量隐藏网页头部、侧栏、聊天和推荐区，只保留播放器铺满右侧舞台，登录二维码、验证码或授权页出现时会自动退出剧场模式，保证二维码完整可扫；直播流使用持久化分区，扫码后的 Bilibili 登录态会在 Gordon 内其它 Bilibili 直播间复用并随应用重启保留；金融快报当前通过主进程调用 Yahoo Finance Chart API 拉取免密公开行情，默认支持黄金期货 `GC=F` 与招商银行 A 股 `600036.SS`，可输入其它 Yahoo Finance symbol 查询，展示最新价、涨跌幅、日高 / 日低、成交量与带价格刻度 / 时间轴的 K 线；当前已支持单日分钟线和小时线，区间包含 `1d`，周期包含 `1m / 5m / 15m / 30m / 60m / 1d / 1wk / 1mo`，连续交易品种的 `1d` 分钟线会按最近 24 小时可视区间展示；K 线横轴会按 Yahoo 返回的交易所时区格式化，并通过可视区间摘要、双层日期/时间刻度和竖向参考线降低跨市场标的的时间误读；黄金类标的会保留原始美元 / 金衡盎司报价，并额外派生 `USD/g` 与可用时的 `CNY/g` 克价；最近快照、常用标的和直播收藏会持久化到 `workflow-library.json` |
...
- 首页保持“左侧 mini 品牌卡 + 图标工具轨 + 右侧工作区”的单主舞台结构
- 桌面端主窗口初始尺寸与最小尺寸统一固定为 `1180x760`，避免工作台在过窄窗口下破坏主舞台布局
- macOS 桌面端主窗口已隐藏原生长标题栏文字，并用浅蓝纸面壳层衔接 Saber Atelier，避免系统标题栏割裂主界面
- 模型管理、能力拓展等次级管理入口统一收进品牌卡设置菜单
- 渲染层已完成 Vue 化，但保留 Gordon 当前桌面工作台的核心布局语义
- 渲染层已建立 feature 目录边界：`shell` 承接工作台壳层配置、左侧导航、首页二次元舞台、Saber 背景轮换、工作台快照启动 / 刷新和根级 watcher，`model-management` 承接模型管理页面组件、编辑状态、余额 / 用量运行态和页面事件 actions，`weekly` 承接任务笔记页面组件、配置、初始状态、任务树 helper、日报 Markdown 生成、模板选择同步、驾驶舱洞察、编辑器同步、自动保存、报表输出状态和页面事件 actions，`command-workshop` 承接命令工坊页面组件、会话状态、附件上下文拼装、Agent 消息转换、执行中 artifact 组装和页面事件 actions，`workflow-library` 承接流程中心页面组件、环境 / 步骤 / 记录草稿、curl / Body 解析、record 转换、运行结果展示 helper 和页面事件 actions，`marketplace` 承接应用广场页面组件、漫画 / 视频项目动作、音乐创作动作与运势解读动作，`writing` 承接 `墨笔生花` 配置、prompt 构建、书稿基础 actions、AI actions 和局部组件，`extensions` 承接能力拓展页面组件、Agent / Skill / MCP / TOOL 编辑状态、Runner 状态和页面事件 actions；后续新增业务逻辑应优先进入对应 feature 模块
- 首页已移除本地 Spline runtime、机器人场景和 Falling Stars 背景层，改为 `HomeAnimeStage.vue` 承接 Saber 专用首页图与轻量品牌文字；旧 `@splinetool/runtime` 依赖已从工程移除
- 桌面端视觉已迁移为 Saber Atelier（二次元工作室）方向，统一冰蓝画布、纸白主工作面、蓝色结构线、金色重点信号、紫色主题识别点和低频背景轮换
- 当前视觉统一已覆盖左侧品牌与主导航、首页二次元舞台、页面标题带、应用入口、KPI、列表项、配置项、按钮、表单控件、详情工具栏、命令工坊消息流和自绘窗口，不改变左侧入口 / 右侧主工作区架构；Saber Atelier 第一阶段去工业化已把高频界面从 HUD / 控制台语汇收敛为纸面工作台、书签细轨、缎带页签和柔和按钮，代码块、K 线图主体、播放器和网页舞台作为深色内容例外保留
- Saber 主题已新增公共可读性覆盖层，统一压住旧深色主题残留的卡片 / 面板 / 抽屉 / 菜单 / 输入 / 按钮 / 标签 / 空态 / 列表行样式；白字只保留在主动作按钮、用户消息、代码块、媒体舞台、封面图和播放器等深色局部
- 内部工作区延续 Saber Atelier 的第二层视觉语言：流程中心入口卡继续使用编号与类型色轨；任务笔记保留周记录信号轨、项目结构轨和状态节点；应用广场入口、项目架、详情侧轨、Tab 和编辑面板统一使用纸白表面、冰蓝边界和暖金字段标签
- 全局滚动条已统一为纤细无边框样式，轨道、角落和按钮保持透明；任务清单、应用广场、模型用量与字段级 AI 等区域只通过滚动条颜色变量做局部区分，不再单独定义粗细或透明边框
- 全局图片放大预览已统一为根层蒙版交互，支持下载、关闭、缩放比例显示、按钮 / 滚轮缩放、拖拽平移和底层操作阻断；漫画素材、漫画章节图片、漫画 AI 输出图与命令工坊图片产物优先接入同一套预览层
- 功能图标已收敛为 `lucide-vue-next` + `GIcon` 语义组件；按钮图标统一走线性、`currentColor` 和状态 token，品牌 / 模型 Logo 继续保留本地 SVG
- 权限申请、删除确认、关键失败提醒与周报模板命名已接入 Gordon 自绘弹窗，继续沿用 Gordon 自绘窗口语义，并切换为纸白表面、蓝色主动作与红色风险态
...
- 已形成“mini 本周驾驶舱 + 当前年份周报列表”的列表态，顶部统计区保持紧凑数据条，列表作为主阅读区吃满剩余高度
- 周报列表右上角已改为按已有年份生成的 mini 年份筛选；默认优先展示当前年份，列表展示该年份下的全部周记录并在区域内滚动，不再只截取最近 5 条
...
- 日报输出区已支持飞书群发送：标题栏时间标签右侧可配置群机器人 Webhook / 签名密钥 / 标题前缀 / 自动日报开关，日报生成按钮右侧提供分享图标按钮，由主进程调用飞书 webhook 发送富文本卡片消息；开启自动日报后，桌面进程会在工作日北京时间 18:30 读取当前 active 周记录、生成今日日报、发送到飞书群并回写最近执行状态；卡片正文使用 `markdown` 元素渲染，卡片标题保持“前缀 + 日期”的精简格式
...
- `流光绘影` 采用“应用入口 -> 项目架 -> 项目详情”的三段式结构，定位为连续短剧 / 短视频生成项目工作台；项目架布局与 `墨笔生花` 书架一致，空列表时会在列表区域中央显示轻量提示；项目详情包含“项目设定 / 镜头规划 / 生成台”三个 tab，支持左侧项目信息折叠、通过“项目导出”弹框导出项目 Markdown、新建镜头，并维护文生视频 / 图生视频、画幅、类型、默认时长、项目概念、视觉与运动风格、分镜规划、镜头说明、参考素材、正向提示词、反向提示词和生成结果；项目设定页已复用 `丹青溢彩` 的素材库交互，可创建人物、物品、场景素材，按视图生成 / 放大 / 下载素材图，并在视图提示词与镜头参考字段中通过 `@素材/视图` 选择引用；视频生成提交会优先把已解析素材图作为首帧 / 参考图走图生视频，STORYBOARD 右侧镜头说明已收敛为紧凑 Shot Brief，生成台已改为左侧视频预览与执行状态、右侧镜头参数编辑器，方便连续镜头切换、复跑和沉淀结果
- `瑶琴映月` 采用“应用入口 -> 专辑列表 -> 专辑编辑详情”的三段式结构，定位为音乐生成与成品管理工具；专辑列表对齐 `墨笔生花` 书架和 `丹青溢彩` 项目架，支持空列表提示和新建专辑；专辑详情对齐项目型编辑页，左侧为可折叠专辑信息轨，底部左侧展示更新时间、右侧提供导出入口，顶部标题位改为全部 / 草稿 / 成品 tab，右侧为曲目编辑与播放器，制作草案收敛为轻量字段而非大标题卡片，瑶音小筑助手抽屉对齐添香小筑 / 灵绘小筑的任务选择、运行和反馈结构；当前支持本地专辑库、专辑信息编辑、专辑 Markdown 导出、曲目草稿 / 成品筛选、曲目编辑、音频 URL 播放器、优先模型生成制作草案，并可通过 `music_gen` 调用 Mureka / Suno 发起歌曲或纯音乐生成；工具会在内部提交并短轮询，拿到音频后直接回填播放器，等待预算耗尽时显示仍在生成并保留任务 ID，查询链路失败时显示 pollFailed 而不伪装为普通 pending
...
- 应用广场已抽出通用 Gordon 处理层 `features/marketplace/marketplaceAgentActions.js`、`features/marketplace/marketplaceAgentContext.js` 与 `features/marketplace/GordonAgentProgress.vue`：丹青溢彩、流光绘影、瑶琴映月和灵犀照命都通过同一层构造默认 Gordon Agent 请求、接收过程事件、展示过程流、提供停止入口、解析图片 / 音频 artifact，并将结果回填到当前应用区域；墨笔生花的添香小筑也复用同一过程流组件。过程流已对齐命令工坊的可见执行语义，按规划、工具执行、授权状态、参数修复、重试 / fallback、主动验证、Skill 和输出整理拆成连续动作块，成功工具默认展示语义动作和目标摘要，错误或授权异常再展开原始输出，避免应用 Gordon 处理长期停在静态“处理中”或被中间结果卡片淹没；当用户明确要求保存、写入、改写或更新应用资产时，运行提示会允许 `dryRun=false` 写回并要求读回验证，没有成功工具结果前不得声称完成。各应用统一保留“快速模式 / Gordon 处理”两种入口，快速模式负责轻量草案或专用工具单步操作，Gordon 处理负责 Agent 主导的质量判断、上下文连续性和必要工具编排。
- 命令工坊、墨笔生花 Gordon 处理和应用广场通用 Gordon 处理已共享工作台资源刷新判断：当 Agent 通过 Application Tools 成功写回应用资产，或通过 Workspace Tools 直接写入 `~/.gord/data/workbench` 下的小说、漫画、视频、音乐、任务、流程、模型、Skill / MCP / TOOL / Agent 配置文件时，前端会主动重拉 workbench snapshot；从命令工坊切回资源型页面时也会后台同步一次，避免工具已改本地资源但页面仍停留在旧 Vue 内存快照。
- `丹青溢彩` Gordon 处理已升级为 `comic` Skill + Application Tools + Gordon Tools 的集成应用：前端提交时显式附加内置 `comic` Skill，并注入当前项目 id、章节 id、更新时间、素材和推荐工具提示；后端 Application Tools 已提供 `comic_create_project / comic_list_projects / comic_read_project / comic_update_project_fields / comic_create_chapter / comic_import_chapters / comic_update_chapter / comic_update_chapter_images / comic_update_assets`，支持按漫画语义新建项目、记录来源元数据、批量导入小说章节/正文、读取项目、预览/写回项目字段、创建实际章节实体、写回已有章节文本、章节分镜轨道、分镜图片和素材库；实际出图仍由 `image_gen` 承接，出图 artifact 可先进入灵绘小筑预览，若 Agent 通过 `comic_*` 工具写回则自动刷新工作台项目状态。
- `丹青溢彩` 小说转漫画链路已补强逐章对照协议：`comic_create_chapter / comic_update_chapter / comic_import_chapters` 均可承载原文章节 `sourceRefs`，命令工坊和应用 Gordon 上下文会要求按章节访问来源、把原文或忠实故事内容写入 `content`、中文章节简介写入 `summary`、中文分镜提示写入 `prompt/storyboards`，并把人物 / 物品 / 场景素材以 `sourceRefs / assetRefs / variants` 关联回章节；前端目录和单章生成页新增紧凑来源对照条，引用素材 chip 会显示版本、适用章节和素材提示词摘要，字段级 AI 也会带入来源、素材和“中文 / 尊重原著 / 不编造”约束。
- 应用广场已新增通用字段级 AI 优化器：`FieldAiOptimizer.vue` 负责编辑框旁的紧凑星光按钮和悬浮面板，`fieldAiActions.js` 负责优先模型调用、停止、替换 / 追加写回；当前已接入墨笔生花、丹青溢彩、流光绘影和瑶琴映月的主要 textarea，并已覆盖丹青溢彩素材视图提示词编辑框；提示词类字段默认生成中文可执行 prompt，并要求忠于原著 / 用户描述，不自动添加未出现的服装、盔甲、披风、武器或装饰，不输出英文 tag 串；丹青溢彩素材视图优化会区分单人三视图、多人/群像设定图、角色关系设定图和阶段/状态参考图，避免把多人组合、坐骑伙伴或同一素材的后续版本误写成单人三视图。后续新增编辑框通过传入字段 id、上下文和 setter 即可复用
- `丹青溢彩` 漫画项目动作已拆入 `features/marketplace/comicActions.js`，包括项目归一化、项目架动作、章节编辑、分镜轨道编辑、素材视图出图、自动保存、删除与 Markdown 导出；章节字段语义已收敛为“章节内容简介 summary / 可选章节正文 content / 来源引用 sourceRefs / 章节级分镜提示 prompt / 分镜轨道 storyboards / 图片 images”，漫画素材支持 `variantLabel / chapterStartIndex / chapterEndIndex / variants` 表达角色造型、服装或场景状态随章节变化，OUTLINE 页右侧按“章节内容简介 / 章节正文 / 分镜与出图提示”三段统一折叠编辑，标题默认保存为“第 N 章”并采用就地编辑，编辑为空时恢复编辑前标题；INTRO 素材库支持 `全部 / 人物 / 物品 / 场景` 分段筛选，新增入口收敛为右侧单个加号按钮并按当前分类创建素材，具体类型仍在右侧详情中维护；素材库支持在每个素材视图上根据视图提示词、素材描述和素材名称直接调用 `image_gen` 生成素材图并回填图片源，素材级提示词不再作为编辑页表单项展示，素材图提示词会按视图规格、识别点、形态 / 材质一致性和负面裁切约束结构化去重，三视图固定使用横图并要求完整全身 / 完整主体，且会明确要求人物等高缩小排版、脚底/鞋底/足骨完整入画、脚底下方留白或地面接触线可见，禁止膝盖、小腿、脚踝、脚掌或脚底裁切；素材视图生成会优先按当前视图标签 / 提示词识别三视图，不再被素材总描述中的关系设定误判为关系图；视图提示词支持 `@素材/视图` 引用已有素材图，命中引用时会把图片传入 `image_gen.images` 走图生图；生成后不会把完整拼装 prompt 反写回视图提示词；对骷髅、骨架、金刚骨等特殊形态角色会进入骨骼本体分支，明确禁止衣服、盔甲、披风、头盔、鞋靴、武器和随机服饰，避免把原著骨骼形态误画成穿甲战士；视图卡片可直接放大或下载，卡片内图片始终按完整宽度正常展示，放大时走全局 `ImageLightbox` 预览层并以 `object-fit: contain` 完整显示、不裁切；CHAPTER 页以分镜轨道为主体，支持新增 / 选择 / 删除分镜，编辑每条分镜的标题、类型、画面内容、对白/旁白、镜头/构图和生图提示词，Canvas 头部提供紧凑“生成单张”快捷按钮，可直接调用 `image_gen` 生成当前分镜单张漫画并追加回当前分镜图片区；右侧 Shot Editor 显式标注“分镜生图提示词”，灵绘小筑可按目标分镜数拆分章节并把图片写回当前分镜
...
- `Search Tools` 已作为独立内置搜索服务接入默认 Agent，提供 `web_search_v2`、`web_research` 与 `github_search_repositories`：支持 Tavily / Brave Search / Serper / SearXNG API provider 和 Bing / Baidu / Google 兜底，能按官方域名扩展查询、去重排序、读取落地页正文、发现页面内相关文档链接，并把结构化来源与正文摘录返回给命令工坊；GitHub 仓库搜索直接返回项目链接、stars、forks、语言、topic、license 和更新时间；默认工具规划在最新事实、资料调研、官方文档和需要引用来源的任务中优先选择 `web_research`，在开源项目查找中优先选择 `github_search_repositories`；Agent Runtime 已补充外部证据硬约束，遇到最新官网价格、官方资料、引用来源或明确联网检索请求时，不接受无工具规划，必要时会自动兜底选择搜索/网页读取工具；Anthropic / Claude 等厂商模型清单和价格任务会优先生成英文官方查询，使用 `includeDomains / preferredDomains` 限定官方域名，并通过 `officialUrls` 把模型总览、官方 Pricing、Release Notes 和关键发布公告作为 pinned sources 交给 `web_research` 在泛搜索前直接读取；工具返回后会检查官方域名、实体相关性和证据充分性，价格问题必须包含价格数字，模型清单问题必须包含模型名称或 API ID，仅命中官网首页、不相关页面、词典页或第三方噪声都不算成功外部证据，会自动换下一条官方 query 或 pinned source 补搜，最终回复在没有有效证据前不得输出伪实时结论
...
- 命令工坊 Agent Runtime 已引入 Resource Registry + Resource Gateway 第一阶段：runtime 会从用户目标、最近会话、任务账本和工具历史中识别 `codebase.project / workspace.filesystem / web.source / desktop.app / artifact.output / media.asset / writing.book / comic.project / conversation.thread` 等资源候选；Resource Resolver 会抽取文件路径、URL、桌面应用、生成任务 ID、artifact、小说书稿、章节、漫画项目和漫画章节等 `resolvedRefs`，并能识别应用广场上下文里的“项目 ID / 当前章节 ID / 书籍 ID”等中文标签；Capability Registry 会按资源类型声明 `codebase.inspect / codebase.edit / codebase.verify / web.research / desktop.inspect / desktop.operate / media.generate / writing.review_continuity / writing.update_asset / comic.read_project / comic.split_storyboard / comic.render_images` 等能力，并把 `resources.primaryResource / candidates / resolvedRefs / capabilityRegistry / capabilityFrame / gatewayPlan / routingPolicy` 注入 Context Packet；Resource Gateway 会把目标资源能力组织成 inspect / act / verify 步骤，提供 `toolBias / argumentHints / verificationBias`，Capability Routing 会读取这些偏置来加权 Planner Tool View，同时避免漫画任务误混写作工具或写作任务误混漫画工具；Planner 系统提示从“工具规划器”升级为“资源任务规划器”，要求先判断资源、能力和 Gateway 步骤，再选择工具，使命令工坊更偏向 Codex / OpenClaw 式桌面 Agent 助手，而不是单纯工具直连
- 命令工坊已引入 Capability Routing（能力路由）与 Planner Tool View：runtime 会基于 Context Packet 推断本轮能力需求，并按能力、动作、执行域、成本、风险、副作用和可逆性对工具做分组排序，注入 Planner、fallback planner 与主动验证规划；Planner 现在只看到可见工具列表，默认隐藏 `join_path / relative_path / normalize_path` 等路径工具以及 `click / type_text / press_key` 等 GUI 原语，优先让模型选择 `writing_* / comic_* / web_research / read_file / get_app_state / image_gen` 等语义更清晰的工具；当本轮存在媒体生成意图时，`image_gen / video_gen / music_gen` 会固定保留在 Planner 可见工具内，避免应用广场上下文或资源偏置把视频 / 音频生成工具挤出白名单
- 命令工坊已补充前置工具规划时延保护：明确生成图片、视频或音乐的请求会在工具发现后直接构造 `image_gen / video_gen / music_gen` 调用计划，跳过通用资源任务规划模型等待；普通工具规划默认最多等待 12 秒，超时后会停止本轮工具规划、保留“前置判断已超时”的过程流记录，并先进入回复整理，避免界面长时间停在“正在选择执行方式”。生成类工具仍会正常进入高风险授权弹窗、执行、产物解析和成功条件验证。Gordon Tools 与 Agent 失败分类层已清洗上游 HTTP HTML 错误页，503 / 502 / 504 / 429 等错误会展示为可读的“上游服务暂时不可用 / 网关异常 / 响应超时 / 限流”摘要，不再把原始 HTML 泄漏到命令工坊时间线。
...
- 命令工坊已支持跨轮任务账本延续：助手消息 artifact 会保留 `taskLedger`，下一次提交会从最近一条助手消息取出账本并作为 `AgentRunRequest.taskLedger` 传给 Agent runtime；runtime 会先通过 Task Continuation Engine 判断 `new_task / continuation / direction_change`，只有明确延续且旧账本仍有可执行状态时才保留上一轮目标、计划、成功条件和 `nextActionHint`，并尝试由 Resource Gateway、Capability Routing 和 required fallback 直接恢复下一步工具调用；新的具体目标或明确转向会创建新任务账本，避免把旧任务状态继续注入当前执行
...
- 命令工坊自动工具链已在 Planner 前加入 Tool Requirement Engine：Runtime 会把最新官网资料 / 价格来源 / URL、workspace 文件与代码、桌面应用、媒体生成、墨笔生花 / 丹青溢彩应用资产等任务标记为 `required`；required 场景下 Planner 的 `shouldCall:false` 会被 Plan Critic 拒绝，Planner 超时或返回无效计划时，Runtime 会按 Capability Routing 的可见工具和 schema 生成 required fallback 工具计划，执行成功后直接进入验证和最终回复，普通 `optional` 任务仍保留原有规划超时后回复整理行为
- 命令工坊自动工具链已在 Planner 前加入 Task Continuation Engine：Planner 现在是低频动作，优先用于新任务、方向变化和不安全恢复；当用户输入“继续 / 接着做 / 继续修 / 把剩下的做完”等延续语句，且上一轮账本有待办、活动计划、待验证成功条件或可执行 `nextActionHint` 时，Runtime 会先从任务状态恢复下一步工具调用，避免每轮重新发现意图、重建完整 Planner 提示并重新推理整个任务
- Verifier 已支持受控主动验证：Runtime 会先根据工具历史对 `tool_result / artifact_created / artifact_exists / command_passed / command_exit_zero / file_contains / file_exists / url_opened / url_matches / ui_state / ui_contains / json_path_equals` 做确定性验证；只有剩余条件属于 `file_exists / file_contains / command_exit_zero / command_passed / url_matches / ui_contains / json_path_equals` 等明确、可行动硬断言，且已有成功工具调用可作为上下文时，才会在最终回复前最多发起 2 轮验证规划；泛化 `tool_result`、已有 artifact 或缺少明确 target / expected 的弱条件不会触发主动验证，主动验证规划超时 / 异常会作为内部降级写入账本，不再进入用户可见复盘过程流
...
- Agent runtime 关键纯逻辑已从 `runtime.ts` 拆到 `ledger.ts`、`resource-registry.ts`、`context-packet.ts`、`capability-router.ts`、`task-continuation.ts`、`tool-requirement.ts`、`evidence-graph.ts`、`plan-critic.ts`、`tool-metadata.ts`、`failure-classifier.ts`、`verifier.ts` 与 `runtime-utils.ts`，并新增 `pnpm run test:agent` 覆盖资源识别、任务延续判定、工具需求判定、required fallback、工具描述清洗、工具画像、Capability Routing、Planner Tool View、失败分类、账本归一化 / 合并、Context Packet、Evidence Graph、Plan Critic、结构化成功条件验证、evidence 生成和 `runAgent` 端到端集成回归；当前集成回归会用本地假模型与假 MCP 服务验证可见工具收敛后仍能选择 Computer Use 执行桌面状态读取，并验证 required workspace 任务在 Planner 超时后不会直接回复整理，且第二轮“继续”可跳过通用 Planner 直接执行 workspace 工具
- 工具失败分类已从“schema / 不可用 / 执行失败”扩展到权限受限、环境状态变化、工具不匹配、时序过早和目标不存在，便于后续 fallback、账本恢复和前端过程流更准确表达失败本质
- `Gordon Tools` 已作为独立本地工具服务接入默认 Agent，会按能力拓展 TOOL 配置动态暴露已启用工具；当前 `image_gen` 可读取 OpenAI 系列图片配置并调用 `imagen` / `imagen/edit`，同步返回图片产物并识别 HTTP 200 内的上游业务错误；`image_gen` 图生图请求会把引用图转换为 multipart 图片文件字段，单图使用 `image`，多图使用 `image[0] / image[1]...`，引用输入支持 URL、data URL、base64、本地路径和 `file://`，响应解析兼容 OpenAI 标准 `data` 与代理 `resp_data / output / images / image_urls / artifacts` 包裹，支持 URL、data URL 和 base64 图片产物；`video_gen` 可读取 Seedance 提交接口、轮询接口、`taskIdPath` 和 `resultUrlPath` 配置，默认走 `/gpt-proxy/volengine/video/submit` 与 `/gpt-proxy/volengine/video/task/{task_id}`，按配置 JSONPath 解析任务 ID 和视频 URL，并提交文生视频、图生视频、首尾帧生视频、参考图 / 视频 / 音频生视频任务，`submit / query` 默认都在工具层执行短轮询并返回 `pending / completed / pollExhausted / pollFailed / pollHistory`，达到预算仍未完成时保留 `taskId` 供后续继续查询；`video_gen` 已区分网络连接超时、健康检查失败和上游接口参数错误，连接层失败时会用假任务查询做不消耗额度的代理健康检查，并把查询类 `curl exit 28 / fetch failed` 归类为可重试网络超时；为避免重复生成或重复扣费，`video_gen submit` 的连接层异常会标记为“提交状态未知”并禁止 Agent 外层盲重试，查询 / 轮询阶段则增加工具内短重试和连续网络抖动容忍；`music_gen` 可读取 Mureka / Suno 配置调用歌曲 / 纯音乐生成和任务查询，Suno 走 `/gpt-proxy/suno/generate` 提交与 POST `/gpt-proxy/suno/detail` 查询，生成和查询默认短轮询到音频 URL 或明确 pending / pollFailed；图片、视频和音频 URL 会归一化为命令工坊可展示产物
- `video_gen` 默认轮询预算已提升为 360 次、总预算 1800000ms；工具 schema、环境变量读取上限和运行时参数归一化保持一致，避免提交成功后因旧的 12 次 / 70 秒预算提前返回 pending
...
- 命令工坊运行态展示已对齐持续执行型 Agent 体验：Planner Tool View、工具数量和工具发现摘要只作为内部 artifact 保留，主消息顶部统一展示“准备工具 / 判断动作 / 执行工具 / 整理回复”的透明内联 live 状态；真实工具步骤会转成面向任务的动作标题（例如 `read_web_page` 显示为“读取网页内容”），工具服务和 tool name 降级为弱化小标签，避免用户看到卡片从“工具数量”突然跳到“Workspace Tools / read_web_page”
- 命令工坊媒体生成意图识别已补充“钢琴曲 / 曲子 / 乐曲 / 音频 / 纯音乐 / BGM / 使用 music_gen”等表达，并支持“确认 / 继续下一步 / 轮询生成结果 / 查询任务状态”等后续指令继承上一轮未完成的生成工具目标，避免可执行生成或异步查询任务被误判为普通聊天；无工具轮次的最终回复提示也不再要求用户手动“进入工具”，而是明确当前未执行的配置、权限或自动工具条件
- Agent 最终回复输出预算已提高到 4096 tokens，避免复杂工具任务在完成后只能给出过短结论
- `image_gen` 已补充上游业务错误识别：即使 HTTP 200，只要返回 `error` 或没有图片数据，也会按工具失败处理，并把过载 / 限流类错误纳入可重试分类；OpenAI 图片模型默认请求 `n=1`、`quality=medium`，图生图会按上游图片接口要求使用 multipart 图片字段上传引用图，并通过 Gordon Tools stderr 日志输出脱敏调用参数和响应摘要
...
- 命令工坊已支持展示工具调用产生的结构化产物，`image_gen / video_gen / music_gen` 返回图片、视频、音频 URL 或 base64 图片时会在助手消息下方直接渲染预览
- 命令工坊执行链路已从纯聊天回合升级为主消息内轻量按需过程流展示：运行中不再固定回显思路摘要和通用执行计划，内部工具规划不单独刷屏，真实工具调用、参数修复、重试 / fallback、停止原因和最终整理才进入可见时间线；成功工具调用默认不外显原始返回，错误、授权异常和停止原因提供折叠回看；外部路径与 Computer Use 授权不再作为独立时间线项插入，而是折叠为对应工具步骤内的状态标签，工具步骤使用前端可见序号避免后端轮次造成跳号，底部不再重复展示关键动作详情链
- 命令工坊高风险工具处理已从“Critic 前置拒绝”改为“用户授权后执行”：`Plan Critic` 不再因为存在待验证成功条件或应用资产写入风险就连续要求修订并停止工具编排；读/列表/检查类 Application Tools 会被识别为低风险只读，写入、生成、执行、删除等有副作用工具会在执行前触发 Gordon 自绘授权窗口，用户允许后继续调用并写回验证，拒绝后以权限受限失败进入恢复链路；命令工坊、应用广场 Gordon 处理和墨笔生花 Gordon 过程流均会展示“工具授权 · 待授权 / 已授权 / 已拒绝”，`无需申请` 模式下的本轮自动通过展示为“自动放行”。
- 命令工坊输入区附件按钮右侧已新增 `访问权限` 紧凑下拉，默认 `无需申请`；本轮 `AgentRunRequest.permissionMode=auto` 会让 Runtime 自动通过外部路径、Computer Use 与高风险工具授权，不弹出确认窗口，也不会把授权写入长期配置；切换为 `按需申请` 后恢复逐次授权确认；会话会持久化当前权限模式，切换会话或归一化配置时不会丢失该选择。
...
- 命令工坊高级设置中的 Agent、Skill、工具服务和工具选择已从原生下拉框切换为 Gordon 自绘紧凑下拉控件，选项弹层统一使用 Saber Atelier 的纸白表面、冰蓝结构边界、蓝色焦点和紧凑列表风格
...
- 支持会话内选择 Agent、附加 Skill、自动工具、限定工具服务 / tool；运行中可继续输入并发送到请求队列，队列在输入框上方按顺序展示，普通队列项会在当前轮完成后按顺序自动进入下一轮；队列项点击“引导”会立即作为用户消息进入当前对话、从队列移除，并通过运行时引导队列静默注入当前 Agent 运行，在下一次工具规划、工具返回、主动验证或最终回复等 Runtime 决策点生效，也支持编辑和删除
- 命令工坊已加强口语查询与笔误容错：`查下 / 查查 / 查一查` 会触发工具编排，Anthropic / Claude 等厂商当前模型清单、官方价格和最新资料会被 Runtime 强制归入外部证据任务；黄金、汇率、股票、原油、比特币等实时行情价格会使用干净的 web research 查询，不再把最近会话上下文拼进搜索词，也不会误 fallback 到 GitHub 仓库搜索；对“旗下罪行的模型”这类明显接近“旗下最新的模型”的输入，会在搜索 query 中归一为最新模型意图，并优先使用官方英文 canonical query、官方域名过滤与官方 pinned source URL；如果搜索命中无关页面、官网首页或缺少价格 / 模型实体证据，Runtime 会自动补搜而不是让最终回复用旧知识兜底。
- 助手消息中可回看工具执行过程、授权与恢复动作、停止原因与链路摘要；工具参数、错误输出和必要展开内容会对敏感字段和媒体大字段做脱敏 / 截断，成功工具原始返回默认留在 artifact 中不占据消息流
...
- 已从“效率工具”正式收敛为 `流程中心 / workflow`，并扩展为“信息雷达 + 金融快报 + 直播流 + API 工作流”的四板块
- `金融快报` 的 K 线数据链路会清洗 Yahoo Finance 返回的空 OHLC（开高低收）值、零价异常点和高低价关系异常点，避免缓存或接口空洞把价格轴拉到 0 后压扁图表；行情工作区已升级为紧凑的“精密行情舱”，使用独立 `FinanceKlineChart.vue` + `lightweight-charts` 展示最多 10000 个有效日内行情点、真实价格 / 时间坐标、成交量副图和最新价格线，时间轴按交易所时区生成规整刻度，并支持十字线 OHLC 读取、滚轮缩放、拖动浏览、MA5 / MA20、成交量开关与图表聚焦模式；进入金融快报工作区后会每 15 秒静默刷新当前标的、区间与周期，顶部同步状态显示“刚刚更新 / N 秒前”，离开页面或窗口隐藏时停止自动刷新
- 首页采用紧凑横向 workflow 启动卡片，保留图标节点、状态摘要、mini 标签和进入箭头；进入不同卡片后切到对应资源工作区，避免把信息流、curl 列表和配置项混在同屏
- 当前默认卡片为 `信息雷达`、`金融快报`、`直播流` 与 `模型接口测试`
- 左侧导航重新进入流程中心时会保留用户离开前的卡片和工作区，例如直播流会回到原播放器舞台；流程中心内部返回按钮仍显式回到首页卡片列表
- `信息雷达` 当前支持多个用户自定义信息窗口，窗口内可维护 RSS、网页、搜索、公众号和手工来源，其中 RSS / Atom、普通网页、公开搜索 RSS 与搜狗微信公众号线索可由桌面主进程手动刷新；刷新会按关键词和排除词过滤、去重、写入信息条目、保留 run history，并在页面内以可折叠左侧窗口轨道 + 紧凑来源 / 标签筛选、相关性分数和普通信息流主舞台展示；条目来源会进入软件内来源阅读页，上层只显示返回、单行标题和外部打开，下方由主进程 `BrowserView` 内嵌网页内容，返回后销毁阅读视图并回到原信息列表。信息雷达初始化不预置具体信息窗口，Agent / 大模型、科研论文、金融、生物学等窗口属于用户配置，统一落在 `~/.gord/data/workbench/workflow-library.json`；公众号来源依赖公开搜索页，只做低频线索发现，冷却期内跳过访问，条目长期只保存标题、作者、发布时间和摘要，不保存搜狗 / 微信临时跳转链接；刷新合并会按公众号标题 + 作者 + 发布时间清理历史重复项，点击打开时再重新按标题 / 作者检索可用链接并加载
...
- `金融快报` 当前支持默认黄金期货 `GC=F`、招商银行 A 股 `600036.SS` 和自定义 Yahoo Finance symbol 查询；主进程通过 Yahoo Finance Chart API 拉取 K 线，渲染层展示最新价、涨跌、日高 / 日低、成交量、区间 / 周期选择和 K 线图，区间支持 `1d` 单日，周期支持 `1m / 5m / 15m / 30m / 60m` 日内分钟与小时线，长区间分钟线会自动收敛到短区间，连续交易品种的 `1d` 分钟线会取最近 24 小时视窗；行情快照会保留交易所时区，K 线横轴、市场时间和蜡烛悬停时间统一按该时区展示，图表标题区会显示可视起止时间、覆盖时长、K 线数量和交易所时区，底部时间轴采用双层日期 / 时间刻度；黄金类标的额外显示按金衡盎司换算的 `USD/g` 克价，并在汇率可用时显示 `CNY/g`；最近快照会回写本地仓储。当前定位为快照查询和观察台，不作为交易级实时行情源
- `直播流` 当前支持固定直播间收藏和临时直播页打开，默认内置 Bilibili 6 号直播间的 `/blanc/6` 纯净播放页；Bilibili 可输入房间号或完整 URL，并统一归一化为 `/blanc/<roomId>`，小红书比赛直播和其它来源按固定直播页 URL 打开；渲染层为左侧直播间轨道 + 平台输入区 + 右侧播放器舞台，主进程通过独立 `BrowserView` 加载直播网页，DOM 可用或短暂兜底后即显示页面，避免直播页长资源加载导致遮罩卡死；Bilibili 直播页会注入播放器剧场模式，隐藏网页头部、侧栏、聊天和推荐等非播放区域，并在识别播放器后隐藏同层级非播放器旁支模块，让播放器画面铺满舞台，若出现登录二维码、验证码或跳转授权页则自动退出剧场模式并抬高登录弹层；直播流使用 `persist:gordon-live-stream` 持久化分区，扫码后的 Bilibili Cookie 会在 Gordon 内部直播间间复用并重启保留，但不直接共享系统 Chrome 登录态；与信息雷达阅读页隔离持久化分区并在切换页面时自动关闭原生视图；后台默认通过 `--log-level=3` 与 `--disable-logging` 静音 Chromium native 日志，减少 Bilibili WebRTC / P2P 探测失败刷屏，可通过 `GORDON_CHROMIUM_DISABLE_LOGGING=0` 临时恢复底层日志；该策略不禁用相关网络能力。当前不默认解析原始 `m3u8 / flv` 流，优先使用平台官方直播页保持播放兼容性
- 流程中心渲染层已拆出 `features/workflow-library/WorkflowLibraryView.vue`、`features/workflow-library/workflowRuntime.js` 与 `features/workflow-library/workflowActions.js`：页面组件承接流程中心首页、信息雷达主界面、金融快报行情台、直播流播放台、信息窗口编辑器、工作流列表、编辑器和执行态模板；runtime 集中承接信息雷达展示 helper、金融行情展示 helper、直播 URL 归一化、curl 解析、请求 Body 修复、环境归一化、记录草稿转换、运行前缺失项检查和运行结果展示 helper；actions 承接选择同步、信息窗口保存 / 删除 / 刷新、金融行情查询、直播间打开 / 收藏 / 外部打开、记录保存 / 复制 / 删除、环境与 Body 写回、执行 / 中断和 IPC 进度回显；`App.vue` 当前只保留顶层状态注入和事件注册
- 流程中心资产当前走本地 JSON 仓储：`~/.gord/data/workbench/workflow-library.json`
- `信息雷达` 交互与内容覆盖已升级：搜索来源改为聚合 Google News RSS + Bing RSS 双源抓取并去重合并，扩大最新技术 / 前沿资讯的覆盖面；信息窗口新增"前沿科技 / AI 前沿 / 开发者 / 科研前沿"四套预设，空态与编辑器均提供一键套用（预设仅作为初始草稿注入，保存后仍归属用户配置，不做项目级硬编码窗口）；信息主舞台保留雷达脉冲实时状态、标题脉冲指示与浅色玻璃预设启动器，信息条目卡片整卡可点直接进入软件内阅读页，键盘可聚焦打开；相关图标经 `GIcon.vue` 语义层扩展（radar / rocket / code / flask / globe / zap / bookmark）
- `信息雷达` 五阶段升级：预设库从 4 个扩充至 11 个，新增算法前沿（arXiv cs.LG / cs.CL / cs.CV / stat.ML + Papers With Code）、全球金融市场（Reuters / FT / Bloomberg / WSJ / MarketWatch）、A 股市场（证监会 / 新浪财经 / 搜索聚合）、政治外交（Reuters World / BBC / AP / Foreign Affairs / 外交部发言）、生物科技（bioRxiv / Nature Biotech / Science Trans Med / arXiv q-bio）、农业食农（FAO / USDA / AgFunder / Food Navigator）、量子与物理（arXiv quant-ph / cond-mat / hep-th / PRL / Nature Physics）七大领域；搜索来源在检测到算法 / 论文 / 生物等学术关键词时自动额外加入 arXiv Atom 搜索，实现三源聚合；每来源最大条数从 16 提升到 32，每窗口最大条数从 160 提升到 400
- `信息雷达` 六阶段升级：新增 `github`（紫色，用于 GitHub Trending / Release Notes RSS）和 `reddit`（橙色，用于 Reddit 社区 RSS）两种来源类型，主进程解析链路直接复用 RSS/Atom feed 解析；算法前沿、开发者、金融市场预设已加入 GitHub Trending 和 r/MachineLearning / r/programming / r/investing / r/economics RSS 来源；信息卡片新增摘要文字展示（2 行截断）、hover 可见的收藏/忽略操作按钮；新增"全部/未读/已收藏"状态 tab 筛选栏，收藏和忽略标记实时持久化到本地仓储；来源类型下拉新增 GitHub / Reddit 选项
...
- TOOL 配置当前内置 `image_gen`、`video_gen`、`music_gen`，分别维护图片、视频、音乐生成能力的供应商启停、默认供应商、API Key、模型 / 能力 ID、接口协议与备注；供应商范围先覆盖 OpenAI / Gemini / 即梦、Seedance / PixVerse / Veo / Sora、Mureka / Suno；`image_gen/openai` 已内置文生图 `imagen`、图生图 `imagen/edit` 端点与 `prompt / model / size / n / quality / image / images` 参数边界，运行时会按单图 `image`、多图 `image[0] / image[1]...` 组装 multipart 图生图请求；`video_gen/seedance` 已改为配置提交接口、轮询接口、`taskIdPath` 和 `resultUrlPath`，默认模型为 `doubao-seedance-2-0-260128`，默认接口为 `/gpt-proxy/volengine/video/submit` 与 `/gpt-proxy/volengine/video/task/{task_id}`，支持文生视频、图生视频、首尾帧生视频、参考图 / 视频 / 音频生视频、异步查询、默认短轮询和无完成结果时的 `pending` 状态回传；`music_gen/mureka` 已内置 `/v1/song/generate`、`/v1/soundtrack/generate`、`/v1/song/query/{task_id}` 与 `/v1/vocal/clone`，`music_gen/suno` 已改为代理提交 `/gpt-proxy/suno/generate` 与 POST 查询 `/gpt-proxy/suno/detail`，生成和查询默认短轮询到音频 URL 或明确 pending / pollFailed；默认 Agent 可通过 `Gordon Tools` 发现并调用已启用的生成能力
...
- 已新增内置 `comic` Skill，目录为 `skills/comic`；它作为漫画创作工作流 Skill，内部约定 `source_importer / project_planner / asset_director / chapter_planner / storyboard_director / image_prompt_director / image_producer / visual_continuity_auditor / project_memory` 节点，并通过 `references/project-workflow.md`、`references/storyboard-system.md`、`references/visual-continuity.md`、`references/image-prompting.md` 承接丹青溢彩项目推进、小说来源导入、素材库、章节分镜、视觉连续性和出图提示词细节。默认 Gordon Agent 已允许该 Skill，丹青溢彩 Gordon 处理会显式绑定该 Skill。
...
- 首页继续使用“左侧 mini 品牌卡 + 左侧图标工具轨 + 右侧单一主工作区”壳层
- 模型管理、能力拓展等次级入口继续挂在品牌卡设置菜单下
- 不再恢复全宽顶部横栏，也不把多个主功能窗口并列摆放
- 后续视觉迭代继续沿用 Saber Atelier，不回退为近黑赛博基底，也不切换为满屏大圆角玻璃卡或单一紫蓝霓虹风格；二次元背景必须服务整体氛围，内容可读性和工作台效率优先
...
- 信息雷达继续以“一个卡片下挂多个信息窗口 + 每个窗口维护来源、过滤规则、摘要规则、条目和刷新历史”为主数据结构；它是桌面 Agent 的信息资源面板，不做成普通门户资讯流

### 5.4 命令工坊

- 保持“列表态 / 对话态”两段式结构，不回到多栏并列主窗格
- 默认 Agent 必须可直接开工，前置配置应继续下沉
- Agent 过程流优先像执行日志一样轻量内联展示，成功工具调用不做大卡片；只有错误、授权异常、停止原因和用户需要复盘的输出才折叠展开
...
- 为模型配置补齐连接测试或校验能力
- 为流程中心继续补齐信息雷达能力：定时刷新、Search Tools 深研接入、公众号订阅源 / 手动剪藏、信息条目保存到 Memory / 任务账本、以及由命令工坊触发的“资料收集 -> 信息窗口沉淀 -> 后续研究”链路
- 为 MCP Server 增加独立测试页、连接诊断与更明确的错误提示
- 为 Agent runtime 继续推进 embedding 召回、长期任务持久化、session 级工具发现缓存、更多领域 Workflow 和更细的验证执行器；当前 Task Continuation Engine、Tool Requirement Engine、Decision Memory、Context Packet、Capability Routing、Plan Critic、Evidence Graph 与结构化成功条件 v2 已先落地，用于把目标、计划、候选工具能力分组、证据、验证状态、已证伪路线、开放问题、执行前质量审查、可引用证据节点和可机器验证断言注入后续执行与规划
- 继续扩展 `Gordon Tools` 的真实多模态生成能力，为 `video_gen` 和 `music_gen` 增加更多供应商返回结构样本的专项回归，并按需接入 PixVerse / Veo / Sora 等后续视频供应商
...
已对齐 Codex / Claude Code 的两批「流畅交互」改造：

- 第一批：流式输出仅在用户贴底时自动跟随、上滚回看不被强制拉回并提供「回到底部」悬浮按钮；最终回复流式阶段改为纯文本 + 闪烁光标承接增量，结束后再走完整富文本渲染，避免每帧整段 Markdown re-parse；运行中 Enter 默认即时插话当前运行、仅在无法注入时退回请求队列；`Esc` 停止本轮、`↑` 在空输入时召回上一条已发送请求
- 第二批：规划 / fallback / 观察压缩 / 主动验证等中间模型调用期间，Runtime 透出带计时的轻量「思考态」心跳（`AgentRunProgressEvent.thinkingText`），长任务静默等待时界面持续可见而不再只有跳动点；运行失败 / 停止从模态弹窗改为内联提示 + 「重试本轮」按钮（保留本轮入参）；停止且账本仍有 `pendingSubtasks / activePlan / nextActionHint` 时提供「从这里继续」入口，复用上一轮 `taskLedger` 由 Task Continuation Engine 续跑；命令工坊内 `Cmd/Ctrl+K` 快速新建会话，代码块沿用 `data-command-copy-code` 一键复制

- 第三批：输入框 `/` 斜杠命令面板，支持过滤、↑/↓ 选择、Enter 执行、Esc 关闭、鼠标点选，内置 `/new`、`/settings`、`/skill`、`/agent`、`/tools-on /tools-off`、`/auto /ask`、`/attach`、`/stop`、`/copy`、`/list` 等命令，覆盖新建会话、打开高级设置、切换工具 / 权限、上传附件、停止运行、复制最后回复和返回列表；首字 / 工具静默等待期间的状态文案加上 shimmer 流光，读起来是「正在思考」而非静止

后续仍可继续打磨：

- 首字延迟期间的骨架占位、附件读取进度提示
- 中间节点「思考态」从计时心跳进一步升级为自然语言推理摘要（当前中间产物多为结构化 JSON，直接流式易产生噪声）
- 斜杠命令面板进一步支持带参数命令（如 `/skill <名称>` 直接定位）
