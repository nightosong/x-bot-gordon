...
- `packages/agent/src/runtime.ts` 当前只保留运行编排、权限、Skill Handler、工具执行和最终回复组装；任务账本、工具元数据、失败分类和成功条件验证分别拆到 `ledger.ts`、`tool-metadata.ts`、`failure-classifier.ts` 与 `verifier.ts`，并通过 `pnpm run test:agent` 覆盖关键纯逻辑回归
