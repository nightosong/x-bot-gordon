...
- 当前能力：
  ...
  - Cyber Signal Deck 视觉已覆盖壳层与核心内部工作区，并完成全局配色增压：流程中心、任务笔记和应用广场在保留业务结构的基础上使用编号轨、类型信号、侧轨索引、紧凑 HUD 面板、状态节点、graphite / void black 深底、电青选中态、翡翠薄荷主操作与琥珀关键状态统一视觉层级
  - 模型管理页面
  - 任务笔记页面
...
- 首页继续保留左侧 mini 品牌卡、设置菜单、紧凑图标工具轨与右侧机器人工作区
...
- 渲染层实现已迁移到 Vue，布局继续保持单一主舞台语义；视觉已收敛为 Cyber Signal Deck（赛博信号甲板），以近黑底色、冰蓝结构线和高辨识状态信号组织信息
...
- 桌面端视觉方向已从偏游戏化的大圆角深色玻璃卡片，调整为 Cyber Signal Deck（赛博信号甲板）
- 统一使用 `Avenir Next` / `Avenir Next Condensed` 与 graphite / void black、冰蓝 / 电青、薄荷绿、翡翠薄荷、琥珀色视觉系统；冰蓝 / 电青负责结构，薄荷绿负责状态、焦点与当前轨道，翡翠薄荷负责主操作，琥珀色负责警示、默认模型与关键数据
- 页面标题优先使用 HUD 式扁平标题带，重复列表项与工具才使用 8px 以内的小圆角面板；首页 3D 机器人保持全幅品牌舞台，并加入角标、刻度、扫描线和信号条等无文字 HUD 装饰
