...
- 内置 `Gordon Tools` 作为能力工具服务接入默认 Agent，会按能力拓展 TOOL 配置动态暴露已启用工具；当前 `image_gen` 可通过 OpenAI 系列图片配置调用 `imagen` / `imagen/edit` 并同步返回图片产物，图生图会按上游要求使用 multipart 图片字段上传引用图，支持 URL、data URL、base64 和本地图片文件输入；`video_gen` 可通过 Seedance 配置提交文生视频、图生视频、首尾帧生视频、参考图生视频任务，`submit / query` 默认都会执行工具层短轮询并把 `pending / completed / pollExhausted / pollFailed / pollHistory` 写入结构化结果，`music_gen` 可通过 Mureka / Suno 配置发起歌曲 / 配乐生成或查询任务，生成与查询同样默认在工具层短轮询并尽量一次调用返回音频 URL
...
- 内置多模态生成能力在 `~/.gord/data/workbench/tool-configs.json` 中维护 TOOL 配置，当前默认包含 `image_gen`、`video_gen`、`music_gen` 三类能力；每类能力维护可用供应商、默认供应商、API Key、Base URL、模型 / 能力 ID 和启停状态；OpenAI 系列图片生成在仓储默认逻辑中内置 `imagen` / `imagen/edit` 端点和工具参数清单，默认使用 `n=1`、`quality=medium`，其中 `imagen/edit` 会把单张引用图作为 multipart `image` 上传，多张引用图作为 `image[0] / image[1]...` 上传；Seedance 视频生成默认按火山方舟官方视频生成 API 调用 `POST /api/v3/contents/generations/tasks` 与 `GET /api/v3/contents/generations/tasks/{task_id}`，支持文生视频、图生视频、首尾帧生视频、参考图 / 视频 / 音频生视频，Base URL 归一化兼容旧代理路径、完整官方任务路径和 `/api/v3` 前缀；Mureka / Suno 音乐生成在仓储默认逻辑中内置歌曲生成、纯音乐 / 配乐生成和任务查询端点，其中 Suno 走代理提交 `/gpt-proxy/suno/generate` 与 POST 查询 `/gpt-proxy/suno/detail`；视频和音乐工具会把提交超时、单次查询超时与轮询总预算分开控制，达到预算时返回 `pending=true`，查询链路失败时返回 `pollFailed=true / pollError`，工具调用时会输出脱敏诊断日志，供后续 Agent 工具调用参考，不在编辑页外显
