...
- 渲染层功能模块目录：`apps/desktop/src/renderer/src/features`
  - `shell`：桌面壳层导航、功能入口、首页二次元舞台、Saber 背景轮换、工作台快照启动 / 刷新与根级 watcher 接线
...
- 当前能力：
  - 基于 Vue 的桌面端工作台壳层
  - 保留“左侧 mini 品牌卡 + 图标工具轨 / 右侧工作区”的主布局
  - 首页二次元舞台、模型管理、任务笔记、流程中心、命令工坊、能力拓展六大页面语义
  - Saber Atelier 视觉已覆盖壳层与核心内部工作区：首页使用 Saber 专用首页图，其它页面通过全局背景层低频轮换 Saber 聊天背景；流程中心、任务笔记和应用广场在保留业务结构的基础上继续使用编号轨、类型信号、侧轨索引、紧凑面板与状态节点，并统一为冰蓝画布、纸白主工作面、蓝色结构线、金色重点信号和少量紫色主题识别点
  - 模型管理页面
  - 任务笔记页面
...
- `任务笔记`、`命令工坊`、`流程中心`、`应用广场` 不再继续把新配置、状态工厂或可复用纯逻辑直接追加到 `App.vue`；新增配置、草稿状态、运行时转换、展示 helper、业务动作和模块页面组件应优先落到对应 `features/<module>/` 下；`App.vue` 当前只保留功能视图选择、少量共享状态和 feature actions 接线，Gordon 弹窗、左侧壳层、首页二次元舞台、Saber 背景轮换、工作台快照启动 / 刷新和根级 watcher 已拆出到 `components/` 与 `features/shell/`
...
- 首页继续保留左侧 mini 品牌卡、设置菜单、紧凑图标工具轨与右侧二次元主舞台
- 品牌卡以 `GORDON` 文字为唯一标识，低频变化复用既有 `BRAND_RANDOM_TEXTS` 候选词库，短暂停留后自动回归；减少动态效果或窗口失焦时保持静态
- 左侧图标工具轨承接首页、应用广场、任务笔记、流程中心、命令工坊的主入口
- 模型管理与能力拓展继续通过品牌卡设置菜单进入
- 右侧工作区继续承接首页舞台、模型管理、任务笔记、命令工坊与能力拓展内容
- 渲染层实现已迁移到 Vue，布局继续保持单一主舞台语义；视觉已迁移为 Saber Atelier（二次元工作室），以 Saber 背景资产、冰蓝画布、纸白主工作面、蓝色结构线、金色重点信号和少量紫色识别点组织信息
...
- 桌面端视觉方向已迁移为 Saber Atelier（二次元工作室）
- 正文使用系统中文 UI 字体，品牌与页面标题使用宋体风格显示字体；配色统一为冰蓝画布、纸白主面、深墨文字、蓝色交互、金色重点与少量淡紫主题识别。蓝色负责焦点与主操作，金色负责提醒、默认模型与关键数据，紫色只用于局部识别
- 非首页每个任务页保留一个纸白主工作面，内部用标题带、分隔线、书签细轨和紧凑账本行建立层级；常规控件保持 32-36px 高、4-6px 圆角与实色按钮。首页二次元舞台保持全幅品牌画面，其它页面通过低频背景轮换仅在边缘和空白区保留主题氛围
